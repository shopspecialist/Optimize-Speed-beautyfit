...
class MenuDrawer extends HTMLElement {
  ...
  openMenuDrawer(summaryElement = false) {
    ...
    if (monster_active) {
      const summaryElement_class = summaryElement ? summaryElement?.getAttribute("class")?.includes("header__icon"):true;
      const summaryElement_aria = summaryElement && summaryElement?.getAttribute("aria-label");
      if (
        !summaryElement_aria &&
        summaryElement_class
      ) {
        setTimeout(() => {
          document
...
class ProductForm extends HTMLElement {
  ...
  onSubmitHandler(evt) {
    const monster_active = !!document.querySelector('script[src*="/cart_renderer."]');
    if(monster_active) return console.log(1)
...
class AddToCart extends HTMLElement {
  ...
  onClickHandler() {
    ...
    if (variantId) {
      if ((document.body.classList.contains('template-cart') || !theme.shopSettings.cartDrawer) && !monster_active) {
...
class BundleProducts extends HTMLElement {
  ...
  onButtonClick(event) {
    event.preventDefault();
    const monster_active = !!document.querySelector('script[src*="/cart_renderer."]');
    if(monster_active) return console.log(3)
