...
a = class {
    ...
    //     if (!this.element.firstElementChild || !this.element.lastElementChild) return;
    ...
    // };
    update = () => {
        // compute slide and scroll-left based values first
        this.slide = this.roundingMethod(this.element.scrollLeft / this.itemSize);
        this.slideScrollLeft = this.slide * this.itemSize;

        // Safely get first/last element children (skip text nodes)
        const firstEl = this.element.firstElementChild;
        const lastEl = this.element.lastElementChild;
        if (!firstEl || !lastEl) {
            // no element children yet — nothing to compute
            this.firstItemInView = true;
            this.lastItemInView = false;
            return;
        }

        // Now safe to call getBoundingClientRect
        const containerRect = this.element.getBoundingClientRect();
        const firstRect = firstEl.getBoundingClientRect();
        const lastRect = lastEl.getBoundingClientRect();

        this.firstItemInView = firstRect.left >= containerRect.left;
        this.lastItemInView = Math.floor(lastRect.right) <= containerRect.right;
    };
    onScrollEnd = () => {
        requestAnimationFrame(() => {
